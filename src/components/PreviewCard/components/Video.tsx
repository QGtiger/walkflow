...
export default function Video() {
  ...
  const { recordingUrl, state, targetStepInfo, setState, hasMoreSteps, reset } = PreviewCardModel.useModel();
  ...
  useEffect(() => {
    ...
    const handlePlay = () => {
      // 精确时间检测逻辑
      animationFrameRef.current = requestAnimationFrame(function checkPlaybackProgress() {
        ...
        // 到达目标时间点时暂停
        if (_latestTargetStepInfo && currentTime >= _latestTargetStepInfo.t - 200) {
        ...
        if (!videoRef.current.paused) {
          animationFrameRef.current = requestAnimationFrame(checkPlaybackProgress);
        }
      });
    };
  ...
  return <video ref={videoRef} src={recordingUrl} controls={false} className="h-full"></video>;
}
